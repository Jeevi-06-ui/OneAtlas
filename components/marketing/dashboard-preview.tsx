...
export function DashboardPreview() {
  ...
  return (
    <div className="rounded-xl border border-border bg-card/90 p-3 shadow-2xl shadow-slate-950/10">
        ...
        <div className="grid gap-3 p-4 lg:grid-cols-[180px_1fr]">
          <aside className="hidden rounded-lg border border-border bg-muted/40 p-3 lg:block">
            {[
              ...
              return (
                <div key={String(label)} className="mb-2 flex items-center gap-2 rounded-md px-2 py-2 text-sm text-muted-foreground first:bg-card first:text-foreground">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              {[
                ...
                <div key={label} className="rounded-lg border border-border bg-card p-4">
                  ...
                  <div className={`mt-3 h-1.5 rounded-full ${metricLines[tone]}`} />
                </div>
              ))}
            </div>
            <div className="grid gap-3 md:grid-cols-[1.1fr_0.9fr]">
              <div className="rounded-lg border border-border bg-card p-4">
                ...
                <div className="mt-6 flex h-40 items-end gap-2">
                  {bars.map((bar, index) => (
                    <div key={index} className="flex flex-1 items-end rounded-t-md bg-sky-500/15">
                      <div className="w-full rounded-t-md bg-sky-500" style={{ height: `${bar}%` }} />
                    </div>
                  ))}
                </div>
              </div>
              <div className="rounded-lg border border-border bg-card p-4">
                ...
                <div className="mt-4 rounded-md border border-border bg-muted/50 p-3 text-sm text-muted-foreground">
